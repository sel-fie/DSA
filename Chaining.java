...
public class Chaining {
    ...
    public int chain(int key) {
        ...
        // [(a*k) mod 2^w] >> (w-r)
        // where >> is the shift right bit operator
        int end = (A * key);
        end = end % power2(w);
        int minus = (w-r);
        end = end/power2(minus);
        return end;
    }
    ...
    public int insertKey(int key){
        //TODO: implement this and change the return statement
        //put key into correct slot, defined by hash function, if collision, count
        int place = chain(key);
        int collisions = 0;
        if(!Table.get(place).isEmpty()){
            collisions = Table.get(place).size();
        }
        Table.get(place).add(key);
        return collisions;
    }
